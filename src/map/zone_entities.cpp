...
void CZoneEntities::EraseStaleDynamicTargIDs()
{
    for (auto it = m_dynamicTargIdsToDelete.begin(); it != m_dynamicTargIdsToDelete.end();)
    {
        ...
        if ((timer::clock::now() - it->second) > 60s)
        {
...
void CZoneEntities::ZoneServer(timer::time_point tick)
{
    ...
    for (const auto* PMob : m_mobsToDelete)
    {
        ...
        {
            m_mobList.erase(itr);
            m_dynamicTargIdsToDelete.emplace_back(PMob->targid, timer::clock::now());
            destroy(PMob);
        }
    }
    ...
    for (const auto* PNpc : m_npcsToDelete)
    {
        ...
        {
            m_npcList.erase(itr);
            m_dynamicTargIdsToDelete.emplace_back(PNpc->targid, timer::clock::now());
            destroy(PNpc);
        }
    }
    ...
    for (const auto* PPet : m_petsToDelete)
    {
        ...
        {
            m_petList.erase(itr);
            m_dynamicTargIdsToDelete.emplace_back(PPet->targid, timer::clock::now());
            destroy(PPet);
        }
    }
    ...
    for (const auto* PTrust : m_trustsToDelete)
    {
        if (auto itr = m_trustList.find(PTrust->targid); itr != m_trustList.end())
        {
            ...
            m_dynamicTargIdsToDelete.emplace_back(PTrust->targid, timer::clock::now());
            destroy(PTrust);
        }
    }
