        ...
        for (auto& predicate : predicateGroup.predicates)
        {
            switch (predicate.condition)
            {
                ...
                case G_CONDITION::SC_AVAILABLE:
                {
                    ...
                    predicateResults.push_back(PSCEffect && PSCEffect->GetStartTime() + 3s < timing_clock::now() && PSCEffect->GetTier() == 0);
                    continue;
                }
        ...
        auto checkTPTrigger = [&]() -> bool
        {
            ...
            switch (tp_trigger)
            {
                ...
                case G_TP_TRIGGER::CLOSER: // Hold TP indefinitely to close a SC.
                {
                    ...
                    return PSCEffect && PSCEffect->GetStartTime() + 3s < timing_clock::now() && PSCEffect->GetTier() == 0;
                    break;
                }
